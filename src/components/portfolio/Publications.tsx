    ...
    <section id="publications" className="border-b border-hairline">
      ...
      <div className="mx-auto max-w-350 px-6 pb-24 md:px-10">
        <article className="relative overflow-hidden border border-foreground bg-background text-foreground shadow-[10px_10px_0_0_var(--color-accent)]">
          {/* Corner mark */}
          <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rotate-12 border border-foreground/10" />
          ...
          <div className="relative grid grid-cols-12 gap-8 p-8 md:p-14">
            ...
            <div className="col-span-12 mt-4 flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={t}
                  className="border border-foreground/30 bg-foreground/5 px-3 py-1.5 font-mono-tech text-[10px] uppercase tracking-widest text-foreground"
                >
                  {t}
                </span>
              ))}
            ...
            <div className="col-span-12 mt-6 grid grid-cols-3 gap-6 border-t border-foreground/10 pt-8">
...
function Metric({ k, v }: { k: string; v: string }) {
  return (
    <div>
      ...
      <p className="mt-2 font-display text-xl font-semibold text-foreground md:text-2xl">{v}</p>
    </div>
  );
}
